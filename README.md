...
切分后的文件以 **Dify 原生 File 变量（blob）** 直接返回，下游节点通过 **Files** 变量接收。无需任何外部上传服务，也无需在插件 Provider 中配置任何 URL。

> 注意：浏览器侧下载文件依赖 Dify 自身 `FILES_URL` 环境变量配置正确（容器内地址与浏览器可访问地址需一致）。下游节点之间传递文件不受此影响。
...
同名字段同步通过 `create_variable_message` 注入 Workflow 变量；文件本体通过 `create_blob_message` 输出。
